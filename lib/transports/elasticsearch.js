...
elasticsearch.prototype.get = function (limit, offset, callback) {
  ...
  self.version('input', function (err) {
    ...
    if (type === 'data') {
      ...
      self.getTemplates(limit, offset, callback)
    } else {
...
elasticsearch.prototype.version = function (prefix, callback) {
  ...
  self.baseRequest(esRequest, function (err, response) {
    if (err) { return callback(err) }
...
elasticsearch.prototype.getTemplates = function (limit, offset, callback) {
  var self = this
  if (self.gotTemplates === true) {
    callback(null, [])
  } else {
    var esRequest = {
      'url': `${self.base.host}/_template/${self.base.index}`,
      'method': 'GET'
    }
    aws4signer(esRequest, self.parent)

    self.baseRequest(esRequest, function (err, response) {
      self.gotTemplates = true
      var payload = []
      if (!err) {
        payload.push(response.body)
      }
...
elasticsearch.prototype.getData = function (limit, offset, callback) {
  ...
  if (self.lastScrollId !== null) {
    ...
    self.baseRequest(searchRequest, function requestResonse (err, response) {
      if (err) {
        callback(err, [])
        return
...
elasticsearch.prototype.set = function (data, limit, offset, callback) {
  ...
  self.version('output', function (err) {
    ...
    if (type === 'data') {
      ...
      self.setTemplates(data, limit, offset, callback)
    } else {
      callback(new Error('unknown type option'), null)
    }
  })
...
elasticsearch.prototype.setMapping = function (data, limit, offset, callback) {
  ...
  if (self.haveSetMapping === true) {
    ...
    self.baseRequest(esRequest, function (err, response) { // ensure the index exists
      if (err) { return callback(err) }
      ...
      for (var index in data) {
        ...
        async.eachSeries(sortedMappings, function (set, done) {
          ...
          self.baseRequest(payload, function (err, response) {
            started--
...
elasticsearch.prototype.setAnalyzer = function (data, limit, offset, callback) {
  ...
  var updateAnalyzer = function (err, response) {
    ...
    for (var index in data) {
      ...
      for (var key in settings) { // iterate through settings
        ...
        self.baseRequest(esRequest, function (err, response, body) {
          if (!err) {
            var bodyError = jsonParser.parse(response.body).error
            if (bodyError) {
              err = bodyError
            ...
            self.baseRequest(payload, function (err, response) { // upload the analysis settings
              started--
              if (!err) {
                var bodyError = jsonParser.parse(response.body).error
                if (bodyError) {
  ...
  if (self.haveSetAnalyzer === true) {
    ...
    self.baseRequest(esRequest, function (err, response) { // ensure the index exists
      ...
      aws4signer(esRequest, self.parent)
      self.baseRequest(esRequest, updateAnalyzer)
    })
  }
}
...
elasticsearch.prototype.setTemplates = function (data, limit, offset, callback) {
  var self = this
  if (self.haveSetTemplates === true) {
    return callback(null, 0)
  }
  ...
  async.forEachOf(data, async.ensureAsync((_template, templateName, cb) => {
    var esRequest = {
      'url': `${self.base.host}/_template/${templateName}`,
      'method': 'PUT',
